...
1. **GitHub owner** for the repo — replaces `<owner>` in `OPERATIONS.md` and is the `GITHUB_FEEDBACK_REPO` value.
2. **`TRUST_PROXY_HOPS=2`** in Vault (nginx on LC2 → Caddy). With the default `0`, `req.ip` is the Caddy container address, so every user shares one rate-limit bucket. If the chain ever changes, this number must change with it.
